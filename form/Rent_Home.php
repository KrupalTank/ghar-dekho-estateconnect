...
$searchRent = isset($_GET['rent']) ? $_GET['rent'] : '';
$searchAddress = isset($_GET['address']) ? $_GET['address'] : '';

// Build the base query
$query = "SELECT * FROM rent_home NATURAL JOIN user_table WHERE 1";

// If there's a rent filter, add it to the query
if (!empty($searchRent)) {
    $query .= " AND rent <= $searchRent";
}
...
<body>
    <div class="page">
        ...
        <main>
            <div class="textarea">
                <h1>....Find Most Suitable Home Here And Live Happy Life....</h1>
            </div>

            <!-- Search Form -->
            <div class="search-form">
                <form action="Rent_Home.php" method="GET">
                    <input type="number" name="rent" placeholder="Max Rent" min="0">
                    <input type="text" name="address" placeholder="Address (e.g., katargam)">
                    <input type="submit" value="Search" name="submit" class="login">
                </form>
            </div>
